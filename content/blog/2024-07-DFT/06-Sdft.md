...
\end{equation}
with the spin-scaling relation (von Barth–Hedin):
\begin{equation}
...
\end{equation}
where $\zeta = (\rho_\uparrow - \rho_\downarrow)/\rho$ is the spin polarisation and $f(\zeta)$
interpolates between the paramagnetic ($\zeta=0$) and fully polarised ($\zeta=1$) limits. GGA
and meta-GGA functionals are extended analogously.
...
\begin{equation}
    \mathbf{V}\_{\rm eff}(\mathbf{r}) = \left[V\_{\rm ext} + V_{\rm H} + V\_{\rm xc}^{(0)}\right]\mathbf{I}
    - \mathbf{B}\_{\rm xc}(\mathbf{r})\cdot\boldsymbol{\sigma},
\end{equation}
where $V_{\rm xc}^{(0)} = \frac{1}{2}(V_{\rm xc}^\uparrow + V_{\rm xc}^\downarrow)$ is the
spin-averaged XC potential and $\mathbf{B}\_{\rm xc} = \frac{1}{2}(V_{\rm xc}^\uparrow - V\_{\rm xc}^\downarrow)\hat{\mathbf{m}}$ is the XC magnetic field aligned along the local magnetisation
...
### Contribution from Orbital Moments

By the **second sum rule of XMCD** (Bruno, 1989):
\begin{equation}
    {\rm MAE} \approx -\frac{\xi}{4\mu_B}(\langle L_z^{\rm easy}\rangle - \langle L_z^{\rm hard}\rangle),
\end{equation}
where $\xi$ is the SOC constant and $\langle L_z\rangle$ is the orbital moment along the
magnetisation axis. Larger orbital moments correlate with larger MAE — a design principle
exploited in rare-earth permanent magnets (Nd$_2$Fe$_{14}$B) and $L1_0$ alloys.
