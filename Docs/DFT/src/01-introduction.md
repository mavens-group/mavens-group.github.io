...
\approx \int \frac{e^2\,\rho(\mathbf{r}')}{|\mathbf{r}\_i-\mathbf{r}'|}\,d\mathbf{r}' \equiv V\_{\rm H}(\mathbf{r}\_i),
\qquad
...
\\[
\hat{H}\_{\rm Hartree} = -\frac{\hbar^2}{2m\_e}\sum\_{i=1}^n\nabla^2\_i + \sum\_i V\_{\rm H}(\mathbf{r}\_i) + V\_{\rm ext}(\mathbf{r})
\\]
...
#### Hartree approximation
Each electron feels the nuclear attraction plus the average repulsion
