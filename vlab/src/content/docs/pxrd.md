...
Diffraction occurs because the typical X-ray wavelength ($\lambda \approx 0.5\text{–}2$ Å) matches the interatomic spacing ($d \approx 1\text{–}5$ Å) in crystals. For a set of lattice planes $(hkl)$ with spacing $d_{hkl}$, constructive interference occurs when:

$$
n\lambda = 2d_{hkl}\sin\theta
$$

where $n$ is the diffraction order, and $\theta$ is the Bragg angle. The spacing $d_{hkl}$ is entirely fixed by the unit cell metric. For common crystal systems:
...
*Figure 1 — Schematic representation of Bragg diffraction illustrating the incident angle $\theta$, the diffraction angle $2\theta$, and the interplanar spacing $d_{hkl}$.*

### Reciprocal Space and Powder Diffraction

Diffraction occurs in reciprocal space. Every diffraction peak corresponds to one reciprocal lattice vector $\mathbf{G} = h\mathbf{a^*} + k\mathbf{b^*} + l\mathbf{c^*}$. Unlike a single crystal, a powder contains millions of randomly oriented crystallites. For every lattice plane, some crystallites automatically satisfy Bragg's condition, producing Debye–Scherrer diffraction rings. The diffractometer records these rings as a 1D plot of intensity vs. $2\theta$.

## Instrumentation and Measurement

Modern laboratory diffractometers utilize Bragg–Brentano geometry. Electrons are accelerated (30–60 kV) into a target (usually Cu, yielding K$\alpha_1 = 1.5406$ Å). The beam passes through incident slits, a monochromator, strikes the flat, randomly oriented powder sample, and diffracts into a receiving slit and a modern hybrid pixel or silicon strip detector.

**Measurement Parameters:** A typical scan ranges from $10^\circ$ to $90^\circ$ $2\theta$, with a step size of $\sim 0.01^\circ$ and counting times of 1–3 hours.
...
Plotting $\beta \cos\theta$ vs. $4\sin\theta$ yields a line where the intercept is size ($1/D$) and the slope is microstrain ($\varepsilon$).
...
*Figure 2 — Standard state-of-the-art data analysis workflow for PXRD.*
