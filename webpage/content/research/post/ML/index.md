...
This is a deliberate trade, and it costs accuracy: where the choice is interpretability against opacity, we take interpretability. A marginal gain in F1 score on a held-out test set is worth less to us than a model whose governing descriptors can be read, criticised, and used to guide the next round of screening or synthesis. The same choice makes the method a natural partner to the group's disorder work: when a training set is built from disordered configurations, an interpretable model is what lets the resulting descriptor be checked against the underlying physics rather than taken on faith.

### Findings
We can already build models that hold high accuracy without surrendering the descriptors that explain it — the combination the methodological commitment demands:
...
### Outlook
The direction is to turn interpretation into prediction across the group's disordered systems. The quantities that govern behaviour in the other three pillars — the exchange topology that fixes a transition temperature, the site statistics that set catalytic activity, the couplings that limit coherence — are exactly what a first-principles calculation returns at high cost, for a handful of compositions at a time. An interpretable model trained on those calculations is a way to carry them across composition space without losing the thread back to the physics: not a surrogate that returns a number and hides its reasoning, but one whose predictive descriptors stay the causal ones. The results above show the models can be built to that standard on real classification problems; the open problem is to train them on disordered configurations and have them point, ahead of any new calculation, to where in composition space the disorder is arranged to do what we want.
